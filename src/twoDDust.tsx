...
export const Dust = () => {
  ...
  useEffect(() => {
    ...
    function drawPersonSilhouette(
      ...
      const runAmp = mood === "run" ? 1 : mood === "panic" ? 1.55 : 0.34;
      const panicAmp = mood === "panic" ? 1 : 0;
      const cadence = mood === "panic" ? 11.2 : mood === "run" ? 8.8 : 4.3;
      const bob = Math.sin(phase * 2.8) * (1.3 * runAmp) + Math.sin(phase * 6.4) * (0.42 + panicAmp * 1.05);
      const bodyLean = (mood === "panic" ? 0.35 : mood === "run" ? 0.2 : 0.04) * facing;
      const panicJitter = panicAmp > 0 ? Math.sin(phase * 31.4) * 0.18 + Math.cos(phase * 24.1) * 0.13 : 0;
      const armSwing = Math.sin(phase * cadence) * (0.92 + panicAmp * 0.8);
      const forearmSwing = Math.sin(phase * (cadence * 1.34) + 0.9) * (0.45 + panicAmp * 0.56);
      const legSwing = Math.sin(phase * (cadence + 1.3) + 0.66) * (1.55 + panicAmp * 0.86);
      const kneeBounce = Math.sin(phase * (cadence * 1.12) + 1.2) * (0.6 + panicAmp * 0.48);
      const shoulderY = y + 8.7 + bob;
      const clothFlicker = Math.sin(phase * (mood === "panic" ? 18 : 9.5) + x * 0.32) * (0.5 + panicAmp * 0.8);
      const breath = mood === "idle" ? Math.sin(phase * 3.6) * 0.36 : Math.sin(phase * 5.2) * 0.2;
      const scarfWave = Math.sin(phase * (mood === "panic" ? 17 : 11.5) + facing * 0.8) * (0.8 + runAmp * 0.6);
      const hipShift = Math.sin(phase * (cadence * 0.5) + 0.7) * (0.36 + runAmp * 0.44);
      const headTilt = (Math.sin(phase * (mood === "idle" ? 2.6 : 6.3)) * 0.06 + panicJitter * 0.04) * (1 + panicAmp * 0.5);

      ctx.save();
      ctx.translate(x + 6, y + 10 + bob);
      ctx.scale(facing * scale, scale);
      ctx.rotate(bodyLean + panicJitter * 0.09 + headTilt * 0.2);
      ctx.translate(-6, -10 - bob);

      const shadow = ctx.createRadialGradient(x + 6, y + 18, 1, x + 6, y + 18, 11.6);
      shadow.addColorStop(0, "rgba(7,11,18,0.44)");
      shadow.addColorStop(1, "rgba(7,11,18,0)");
      ctx.fillStyle = shadow;
      ctx.fillRect(x - 6, y + 14, 24, 11);

      const silhouetteBack = ctx.createLinearGradient(x + 1.5, y + 6, x + 10.8, y + 15.2);
      silhouetteBack.addColorStop(0, "rgba(22, 32, 48, 0.4)");
      silhouetteBack.addColorStop(1, "rgba(12, 20, 34, 0.2)");
      ctx.fillStyle = silhouetteBack;
      ctx.beginPath();
      ctx.moveTo(x + 2.2, y + 6.2);
      ctx.lineTo(x + 9.8, y + 6.2);
      ctx.lineTo(x + 10.8, y + 14.6);
      ctx.lineTo(x + 1.4, y + 14.6);
      ctx.closePath();
      ctx.fill();

      const pack = ctx.createLinearGradient(x + 1.6, y + 7.1, x + 4.8, y + 14.8);
      pack.addColorStop(0, "rgba(34, 46, 66, 0.58)");
      pack.addColorStop(1, "rgba(14, 22, 36, 0.32)");
      ctx.fillStyle = pack;
      ctx.beginPath();
      ctx.moveTo(x + 2.0, y + 7.1);
      ctx.lineTo(x + 4.8, y + 7.1);
      ctx.lineTo(x + 4.4, y + 14.2);
      ctx.lineTo(x + 1.6, y + 14.2);
      ctx.closePath();
      ctx.fill();

      ctx.strokeStyle = `rgba(255, 220, 188, ${0.14 + panicAmp * 0.2})`;
      ctx.lineWidth = 1.1;
      ctx.beginPath();
      ctx.moveTo(x + 3.8, y + 8.2 + breath);
      ...
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(x + 4.7, y + 13.1);
      ctx.lineTo(x + 3.7 - legSwing * 0.72, y + 15.6 + kneeBounce * 0.52);
      ctx.lineTo(x + 3.25 - legSwing * 0.94, y + 18.25 + Math.abs(kneeBounce) * 0.42);
      ctx.moveTo(x + 7.2, y + 13.1);
      ctx.lineTo(x + 8.2 + legSwing * 0.75, y + 15.45 - kneeBounce * 0.48);
      ctx.lineTo(x + 8.95 + legSwing * 0.98, y + 18.25 + Math.abs(kneeBounce) * 0.24);
      ctx.stroke();

      ctx.strokeStyle = "rgba(248, 232, 206, 0.24)";
      ctx.lineWidth = 1.05;
      ctx.beginPath();
      ctx.moveTo(x + 4.7, y + 13.1);
      ctx.lineTo(x + 3.7 - legSwing * 0.72, y + 15.6 + kneeBounce * 0.52);
      ...
      ctx.fillStyle = "rgba(52, 30, 18, 0.66)";
      ctx.fillRect(x + 1.95 - legSwing * 0.74, y + 18.1, 2.35, 1.15);
      ctx.fillRect(x + 8.15 + legSwing * 0.74, y + 18.1, 2.35, 1.15);

      const torso = ctx.createLinearGradient(x + 1.9, y + 5.8, x + 10.2, y + 15.2);
      torso.addColorStop(0, palette.clothA);
      torso.addColorStop(0.52, palette.clothB);
      torso.addColorStop(1, "rgba(40, 22, 16, 0.95)");
      ctx.fillStyle = torso;
      ctx.beginPath();
      ctx.moveTo(x + 2.0, y + 6.05);
      ctx.lineTo(x + 10.0, y + 6.05);
      ctx.lineTo(x + 10.55, y + 13.95);
      ctx.lineTo(x + 1.6, y + 13.95);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.fillRect(x + 2.2, y + 11.7, 7.8, 1.35);
      ctx.fillStyle = "rgba(250, 220, 178, 0.22)";
      ctx.fillRect(x + 2.35, y + 10.25 + breath * 0.2, 7.6, 0.65);
      ctx.fillStyle = "rgba(34, 24, 20, 0.42)";
      ctx.fillRect(x + 2.45, y + 12.55 + hipShift * 0.2, 7.2, 0.78);

      ctx.fillStyle = `rgba(255, 224, 194, ${0.17 + panicAmp * 0.24})`;
      ctx.beginPath();
      ctx.moveTo(x + 8.9, y + 8.2);
      ctx.quadraticCurveTo(
        x + 10.9 + clothFlicker * 0.55,
        y + 9.55,
        x + 12.2 + clothFlicker,
        y + 10.25 + panicAmp * 0.52,
      );
      ctx.quadraticCurveTo(x + 10.55, y + 10.18, x + 8.6, y + 9.05);
      ctx.closePath();
      ctx.fill();

      ctx.fillStyle = `rgba(255, 206, 176, ${0.1 + panicAmp * 0.15})`;
      ctx.beginPath();
      ...
      ctx.lineTo(x + 1.15 - armSwing * 0.94, y + 10.5 + Math.abs(armSwing) * 0.44);
      ctx.lineTo(x + 0.35 - armSwing * 1.08 - forearmSwing * 0.66, y + 12.15 + Math.abs(forearmSwing) * 0.48);
      ctx.moveTo(x + 9.6, shoulderY + 0.2);
      ctx.lineTo(x + 10.65 + armSwing * 0.96, y + 10.62 + Math.abs(armSwing) * 0.36);
      ctx.lineTo(x + 11.45 + armSwing * 1.12 + forearmSwing * 0.58, y + 12.22 + Math.abs(forearmSwing) * 0.38);
      ctx.stroke();

      ctx.fillStyle = "rgba(252, 236, 212, 0.28)";
      ctx.beginPath();
      ctx.arc(x + 0.35 - armSwing * 1.08 - forearmSwing * 0.66, y + 12.15, 0.78, 0, Math.PI * 2);
      ctx.arc(x + 11.45 + armSwing * 1.12 + forearmSwing * 0.58, y + 12.22, 0.78, 0, Math.PI * 2);
      ctx.fill();

      const headGlow = ctx.createRadialGradient(x + 6.1, y + 3.8, 1, x + 6.1, y + 3.8, 7.8);
      headGlow.addColorStop(0, "rgba(255,236,196,0.26)");
      headGlow.addColorStop(1, "rgba(255,236,196,0)");
      ctx.fillStyle = headGlow;
      ctx.fillRect(x - 2.4, y - 4.8, 17.5, 15.4);

      ctx.fillStyle = palette.skin;
      ctx.beginPath();
      ctx.ellipse(x + 6.12, y + 3.62, 3.34, 3.62, 0, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = palette.hair;
      ctx.beginPath();
      ctx.ellipse(x + 6.3, y + 2.16, 3.76, 2.2, -0.1, Math.PI, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(x + 3.78, y + 3.2, 1.24, 1.68, -0.2, 0, Math.PI * 2);
      ctx.fill();

      for (let i = 0; i < 3; i++) {
        const strand = phase * (8 + i * 2.5) + i * 0.8;
        const sx = x + 7.2 + i * 0.65;
        const sy = y + 1.8 + i * 0.28;
        ctx.strokeStyle = `rgba(34, 22, 16, ${0.28 - i * 0.06})`;
        ctx.lineWidth = 0.65;
        ctx.beginPath();
        ctx.moveTo(sx, sy);
        ctx.quadraticCurveTo(sx + Math.sin(strand) * 0.6, sy + 0.8, sx + Math.sin(strand * 1.2) * 0.8, sy + 1.8 + panicAmp * 0.4);
        ctx.stroke();
      }

      const eyeSpark = 0.5 + Math.sin(phase * 15 + x * 0.72) * 0.3;
      ctx.fillStyle = `rgba(18,26,38,${0.58 + eyeSpark * 0.34})`;
      ctx.fillRect(x + 7.12, y + 3.45, 0.95, 0.95);
      ctx.fillStyle = `rgba(255, 250, 244, ${0.18 + eyeSpark * 0.18})`;
      ctx.fillRect(x + 7.48, y + 3.64, 0.22, 0.22);

      ctx.fillStyle = `rgba(255, 198, 178, ${0.13 + panicAmp * 0.11})`;
      ctx.fillRect(x + 7.82, y + 4.66, 1.08, 0.86);

      ctx.strokeStyle = `rgba(28, 20, 16, ${0.3 + panicAmp * 0.34})`;
      ctx.lineWidth = 0.8;
      ctx.beginPath();
      ctx.moveTo(x + 6.7, y + 2.9);
      ctx.lineTo(x + 7.95, y + 2.65 - panicAmp * 0.25);
      ctx.stroke();

      ctx.strokeStyle = `rgba(120, 52, 44, ${0.24 + panicAmp * 0.28})`;
      ctx.lineWidth = 0.9;
      ctx.beginPath();
      if (mood === "panic") {
        ctx.moveTo(x + 7.1, y + 5.95);
        ctx.quadraticCurveTo(x + 8.3, y + 6.7 + panicAmp * 0.45, x + 9.25, y + 5.95);
      } else if (mood === "run") {
        ctx.moveTo(x + 7.3, y + 5.8);
        ctx.quadraticCurveTo(x + 8.2, y + 6.1, x + 8.95, y + 5.85);
      } else {
        ctx.moveTo(x + 7.4, y + 5.85);
        ctx.quadraticCurveTo(x + 8.2, y + 5.95 + breath * 0.15, x + 8.9, y + 5.8);
      }
      ctx.stroke();
      ...
      if (mood === "panic") {
        ...
        ctx.lineWidth = 1.3;
        ctx.beginPath();
        ctx.moveTo(x + 10.85, y + 1.2);
        ctx.lineTo(x + 13.3, y - 1.22);
        ctx.moveTo(x + 12.82, y + 1.9);
        ctx.lineTo(x + 15.25, y - 0.45);
        ctx.stroke();

        const puff = Math.sin(phase * 16 + x) * 0.5 + 0.5;
        ctx.fillStyle = `rgba(246, 250, 255, ${0.1 + puff * 0.14})`;
        ctx.beginPath();
        ctx.ellipse(x + 10.2 + puff * 0.9, y + 4.4, 1.45 + puff * 0.85, 0.92 + puff * 0.54, -0.2, 0, Math.PI * 2);
        ctx.fill();
      }
    ...
    function draw(dt: number) {
      ...
      if (tool.falling.length) {
        const tFall = now * 0.006;
        for (let i = 0; i < tool.falling.length; i++) {
          const f = tool.falling[i];
          const fx = f.x - camX;
          const fy = f.y - camY;
      ...
      // persistent carried dirt-ball: major deformation + vortex chunk swarm + impact shell
      ...
      if (visibleBlob > 0.35) {
        ...
        const velocityInfluence = Math.min(1, Math.hypot(player.vx, player.vy) / 420);
        const squash = 1 + Math.sin(tBlob * 1.8) * 0.2 + pulse * 0.34 + velocityInfluence * 0.12;
        const stretch = 1 - Math.sin(tBlob * 1.8) * 0.14 - pulse * 0.18;

        ctx.save();
        ctx.globalCompositeOperation = "lighter";

        const ribbonCount = 6 + Math.floor(Math.min(1, visibleBlob / 14) * 8);
        for (let r = 0; r < ribbonCount; r++) {
          const rf = r / Math.max(1, ribbonCount - 1);
          const rt = tBlob * (1.7 + rf * 0.85) + r * 0.62;
          const radius = visibleBlob * (0.95 + rf * 0.9 + pulse * 0.35);
          const rx = bx + Math.cos(rt) * radius * (0.75 + velocityInfluence * 0.25);
          const ry = by + Math.sin(rt * 1.18) * radius * 0.48;
          const rw = visibleBlob * (0.28 + (1 - rf) * 0.24);
          const rh = rw * (0.46 + Math.sin(rt * 1.3) * 0.2 + pulse * 0.12);
          ctx.fillStyle = `rgba(${196 + (r % 3) * 18}, ${152 + (r % 2) * 16}, ${98 + (r % 4) * 8}, ${0.08 + (1 - rf) * 0.22 + pulse * 0.13})`;
          ctx.beginPath();
          ctx.ellipse(rx, ry, rw, rh, rt * 0.42, 0, Math.PI * 2);
          ctx.fill();
        }

        const trailCount = 8 + Math.floor(Math.min(1, visibleBlob / 16) * 8);
        for (let i = 0; i < trailCount; i++) {
          const tt = i / Math.max(1, trailCount - 1);
          const txTrail = bx - (player.vx * 0.036 + Math.cos(tBlob + i * 0.7) * 2.4) * (tt * 3.1);
          const tyTrail = by - (player.vy * 0.026 + Math.sin(tBlob * 1.4 + i * 0.75) * 2) * (tt * 2.4);
          const tr = visibleBlob * (0.62 - tt * 0.45);
          ctx.fillStyle = `rgba(214, 176, 122, ${0.11 + (1 - tt) * 0.2 + pulse * 0.16})`;
          ctx.beginPath();
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by, (visibleBlob + pulse * 4.4) * squash, (visibleBlob + pulse * 3.1) * stretch, Math.sin(tBlob * 0.95) * 0.45, 0, Math.PI * 2);
        ctx.fill();

        const shellCount = 3;
        for (let sIdx = 0; sIdx < shellCount; sIdx++) {
          const sf = sIdx / (shellCount - 1 || 1);
          const inner = ctx.createRadialGradient(
            bx - visibleBlob * (0.28 - sf * 0.1),
            by - visibleBlob * (0.28 - sf * 0.08),
            1,
            bx,
            by,
            visibleBlob * (1.2 - sf * 0.25),
          );
          inner.addColorStop(0, `rgba(244, 204, 146, ${0.35 - sf * 0.08 + pulse * 0.18})`);
          inner.addColorStop(0.62, `rgba(170, 124, 76, ${0.3 - sf * 0.06 + pulse * 0.14})`);
          inner.addColorStop(1, "rgba(84, 58, 34, 0)");
          ctx.fillStyle = inner;
          ctx.beginPath();
          ctx.ellipse(
            bx + Math.cos(tBlob * (1.1 + sf * 0.6)) * (1 + sf * 1.6),
            by + Math.sin(tBlob * (1.3 + sf * 0.5)) * (0.8 + sf * 1.2),
            visibleBlob * (0.98 - sf * 0.16),
            visibleBlob * (0.74 - sf * 0.12),
            Math.sin(tBlob * (0.9 + sf * 0.5)) * 0.6,
            0,
            Math.PI * 2,
          );
          ctx.fill();
        ...
        const orbitChunks = 16 + Math.floor(Math.min(1, visibleBlob / 14) * 14);
        for (let i = 0; i < orbitChunks; i++) {
          const af = i / orbitChunks;
          const a = tBlob * (2.3 + pulse * 1.6) + af * Math.PI * 2;
          const wobble = Math.sin(tBlob * 3 + i * 0.8) * 0.28 + Math.cos(tBlob * 2.3 + i * 0.45) * 0.22;
          const swirlBias = Math.sin(tBlob * 1.4 + af * Math.PI * 4) * 0.12;
          const r = visibleBlob * (0.95 + wobble + pulse * 0.24 + velocityInfluence * 0.14 + swirlBias);
          const cx = bx + Math.cos(a) * r;
          const cy = by + Math.sin(a * 1.5 + wobble) * (r * 0.62);
          const sz = 1.1 + (i % 4) * 0.62 + pulse * 1.18;
          ctx.fillStyle = `rgba(${150 + (i % 3) * 26}, ${108 + (i % 4) * 14}, ${70 + (i % 2) * 12}, ${0.5 + pulse * 0.3})`;
          ctx.beginPath();
          ctx.arc(cx, cy, sz, 0, Math.PI * 2);
          ctx.fill();

          if (i % 3 === 0) {
            ctx.fillStyle = `rgba(246, 220, 176, ${0.13 + pulse * 0.16})`;
            ctx.beginPath();
            ctx.arc(cx - sz * 0.2, cy - sz * 0.2, Math.max(0.45, sz * 0.35), 0, Math.PI * 2);
            ctx.fill();
          }

          if (i % 5 === 0) {
            ctx.strokeStyle = `rgba(236, 196, 144, ${0.08 + pulse * 0.14})`;
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(cx, cy);
            ctx.lineTo(cx - Math.cos(a) * (4 + pulse * 7), cy - Math.sin(a * 1.4) * (3 + pulse * 5));
            ctx.stroke();
          }
        }

        const ringCount = 3 + Math.floor(Math.min(1, visibleBlob / 18) * 3);
        for (let r = 0; r < ringCount; r++) {
          const rf = r / Math.max(1, ringCount - 1);
          const pulseWave = Math.sin(tBlob * (2.4 + rf) + r * 1.1) * 0.5 + 0.5;
          const ringR = visibleBlob * (1.05 + rf * 0.95 + pulse * 0.6 + pulseWave * 0.22);
          ctx.strokeStyle = `rgba(226, 184, 132, ${0.08 + (1 - rf) * 0.12 + pulse * 0.13})`;
          ctx.lineWidth = 1.1 - rf * 0.35;
          ctx.beginPath();
          ctx.ellipse(bx, by, ringR, ringR * (0.58 + Math.sin(tBlob * 1.3 + r) * 0.08), Math.sin(tBlob * 0.9 + r) * 0.35, 0, Math.PI * 2);
          ctx.stroke();
        }

        const tendrilCount = 6 + Math.floor(Math.min(1, visibleBlob / 12) * 8);
        ctx.strokeStyle = `rgba(234, 196, 144, ${0.1 + pulse * 0.2 + velocityInfluence * 0.08})`;
        ctx.lineWidth = 1;
        for (let i = 0; i < tendrilCount; i++) {
          const tf = i / Math.max(1, tendrilCount - 1);
          const ta = tBlob * (1.3 + tf * 0.7) + tf * Math.PI * 2;
          const innerR = visibleBlob * (0.9 + Math.sin(tBlob * 2 + i) * 0.2);
          const outerR = visibleBlob * (1.9 + pulse * 1.4 + Math.sin(tBlob * 1.6 + i * 0.6) * 0.3);
          const sx = bx + Math.cos(ta) * innerR;
          const sy = by + Math.sin(ta * 1.2) * innerR * 0.64;
          const cx1 = bx + Math.cos(ta + 0.45) * (innerR + (outerR - innerR) * 0.35);
          const cy1 = by + Math.sin((ta + 0.45) * 1.2) * (innerR + (outerR - innerR) * 0.35) * 0.62;
          const ex = bx + Math.cos(ta + 0.72) * outerR;
          const ey = by + Math.sin((ta + 0.72) * 1.2) * outerR * 0.58;
          ctx.beginPath();
          ctx.moveTo(sx, sy);
        ...
        const swirlPasses = 2;
        for (let pass = 0; pass < swirlPasses; pass++) {
          const alphaBase = 0.05 + pass * 0.03 + pulse * 0.08;
          ctx.strokeStyle = `rgba(${236 - pass * 18}, ${194 - pass * 14}, ${140 - pass * 10}, ${alphaBase})`;
          ctx.lineWidth = 1.1 - pass * 0.25;
          ctx.beginPath();
          const strands = 14 + Math.floor(visibleBlob * 0.6);
          for (let i = 0; i < strands; i++) {
            const a = (i / strands) * Math.PI * 2 + tBlob * (1.6 + pass * 0.4);
            const r0 = visibleBlob * (0.9 + pass * 0.25);
            const r1 = visibleBlob * (1.7 + pulse * 0.9 + pass * 0.25);
            const x0 = bx + Math.cos(a) * r0;
            const y0 = by + Math.sin(a * 1.3) * r0 * 0.62;
            const x1 = bx + Math.cos(a + 0.5) * ((r0 + r1) * 0.5);
            const y1 = by + Math.sin((a + 0.5) * 1.3) * ((r0 + r1) * 0.5) * 0.62;
            const x2 = bx + Math.cos(a + 1) * r1;
            const y2 = by + Math.sin((a + 1) * 1.3) * r1 * 0.62;
            ctx.moveTo(x0, y0);
        ...
        if (pulse > 0.04) {
          const shell = ctx.createRadialGradient(bx, by, visibleBlob * 0.5, bx, by, visibleBlob * (2.3 + pulse * 1.8));
          shell.addColorStop(0, `rgba(255, 220, 168, ${0.18 + pulse * 0.24})`);
          shell.addColorStop(0.55, `rgba(214, 166, 112, ${0.1 + pulse * 0.16})`);
          shell.addColorStop(1, "rgba(148, 98, 60, 0)");
          ctx.fillStyle = shell;
          ctx.beginPath();
          ctx.arc(bx, by, visibleBlob * (2.2 + pulse * 1.6), 0, Math.PI * 2);
          ctx.fill();

          const impactSpokes = 5 + Math.floor(pulse * 6);
          ctx.strokeStyle = `rgba(255, 226, 182, ${0.08 + pulse * 0.2})`;
          ctx.lineWidth = 1.2;
          ctx.beginPath();
          for (let i = 0; i < impactSpokes; i++) {
            const a = (Math.PI * 2 * i) / impactSpokes + tBlob * 0.45;
            const innerR = visibleBlob * (1.2 + pulse * 0.5);
            const outerR = visibleBlob * (2.1 + pulse * 1.3);
            ctx.moveTo(bx + Math.cos(a) * innerR, by + Math.sin(a) * innerR * 0.7);
            ctx.lineTo(bx + Math.cos(a) * outerR, by + Math.sin(a) * outerR * 0.8);
          }
          ctx.stroke();
        }
      ...
      const panicPulse = mood === "panic" ? 0.4 + Math.sin(now * 0.02) * 0.3 : 0;
      if (panicPulse > 0.02) {
        ctx.strokeStyle = `rgba(255, 210, 188, ${0.08 + panicPulse * 0.16})`;
        ctx.lineWidth = 1.2;
        for (let i = 0; i < 3; i++) {
          const rr = 8 + i * 6 + panicPulse * 8;
          ctx.beginPath();
          ctx.ellipse(px + player.w * 0.5, py + player.h * 0.5, rr * 0.85, rr, 0, 0, Math.PI * 2);
          ctx.stroke();
        }
      }

      if (runBlend > 0.12) {
        const strideTrail = 2 + Math.floor(runBlend * 4);
        const runDir = player.vx >= 0 ? -1 : 1;
        for (let i = 0; i < strideTrail; i++) {
          const tt = i / Math.max(1, strideTrail - 1);
          const ax = px + player.w * 0.5 + runDir * (3 + tt * 8);
          const ay = py + player.h * (0.35 + tt * 0.4);
          ctx.fillStyle = `rgba(220, 238, 255, ${0.04 + (1 - tt) * 0.1 * runBlend})`;
          ctx.beginPath();
          ctx.ellipse(ax, ay, 2.6 - tt * 1.2, 1.2 - tt * 0.4, 0, 0, Math.PI * 2);
          ctx.fill();
        }

        const runRibbonCount = 3 + Math.floor(runBlend * 5);
        ctx.strokeStyle = `rgba(196, 228, 255, ${0.05 + runBlend * 0.14})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
        for (let i = 0; i < runRibbonCount; i++) {
          const rf = i / Math.max(1, runRibbonCount - 1);
          const ry = py + player.h * (0.22 + rf * 0.58);
          const len = 8 + runBlend * 20;
          const wob = Math.sin(now * 0.01 + i * 0.8) * (0.8 + runBlend * 1.8);
          const fromX = px + player.w * 0.45;
          const toX = fromX - Math.sign(player.vx || 1) * len;
          ctx.moveTo(fromX + wob, ry);
          ctx.lineTo(toX + wob * 0.6, ry + Math.sin(now * 0.008 + i) * 1.2);
        }
        ctx.stroke();
      }

      if (panicBlend > 0.3) {
        const panicLines = 3 + Math.floor(panicBlend * 5);
        ctx.strokeStyle = `rgba(255, 210, 188, ${0.05 + panicBlend * 0.12})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
        for (let i = 0; i < panicLines; i++) {
          const a = now * 0.008 + i * 1.2;
          const r = 9 + i * 3 + panicBlend * 5;
          const x0 = px + player.w * 0.5 + Math.cos(a) * r;
          const y0 = py + player.h * 0.5 + Math.sin(a * 1.2) * r * 0.65;
          const x1 = x0 + Math.cos(a) * (4 + panicBlend * 8);
          const y1 = y0 + Math.sin(a * 1.1) * (3 + panicBlend * 6);
          ctx.moveTo(x0, y0);
          ctx.lineTo(x1, y1);
        }
        ctx.stroke();
      ...
      const heroEnergy = clamp01(runBlend * 0.5 + panicBlend * 0.55 + velocityEnergy * 0.35);
      if (heroEnergy > 0.12) {
        const corona = ctx.createRadialGradient(px + player.w * 0.5, py + player.h * 0.5, 3, px + player.w * 0.5, py + player.h * 0.5, 24 + heroEnergy * 20);
        corona.addColorStop(0, `rgba(224, 242, 255, ${0.03 + heroEnergy * 0.05})`);
        corona.addColorStop(0.55, `rgba(156, 206, 255, ${0.02 + heroEnergy * 0.05})`);
        corona.addColorStop(1, "rgba(84, 132, 202, 0)");
        ctx.fillStyle = corona;
        ctx.fillRect(px - 26, py - 24, player.w + 52, player.h + 48);

        const wakeCount = 4 + Math.floor(heroEnergy * 6);
        ctx.strokeStyle = `rgba(206, 230, 255, ${0.04 + heroEnergy * 0.1})`;
        ctx.lineWidth = 1;
        for (let i = 0; i < wakeCount; i++) {
          const rf = i / Math.max(1, wakeCount - 1);
          const rr = 9 + rf * 16 + Math.sin(now * 0.01 + i * 0.9) * 1.8;
          ctx.beginPath();
          ctx.ellipse(px + player.w * 0.5, py + player.h * 0.5, rr * (0.75 + rf * 0.1), rr, Math.sin(now * 0.003 + i) * 0.2, 0, Math.PI * 2);
          ctx.stroke();
        }
      }
      ...
      if (quest.state === "wave") {
        ...
        const surgeGlow = ctx.createRadialGradient(
          canvasEl.width * 0.5,
          canvasEl.height * 0.55,
          30,
          canvasEl.width * 0.5,
          canvasEl.height * 0.55,
          Math.max(canvasEl.width, canvasEl.height) * 0.65,
        );
        surgeGlow.addColorStop(0, `rgba(210, 242, 255, ${0.04 + shock * 0.08})`);
        surgeGlow.addColorStop(0.55, `rgba(120, 182, 246, ${0.03 + shock * 0.07})`);
        surgeGlow.addColorStop(1, "rgba(16, 38, 78, 0)");
        ctx.fillStyle = surgeGlow;
        ctx.fillRect(0, 0, canvasEl.width, canvasEl.height);

        const impactDebris = 20 + Math.floor(shock * 48);
        ctx.strokeStyle = `rgba(232, 244, 255, ${0.06 + shock * 0.14})`;
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        for (let i = 0; i < impactDebris; i++) {
          const px = ((i * 53.7 + now * (0.11 + shock * 0.16)) % (canvasEl.width + 160)) - 80;
          const py = ((i * 39.1 + now * (0.14 + shock * 0.22)) % (canvasEl.height + 120)) - 60;
          const len = 4 + shock * 10;
          const tilt = -1.5 - shock * 4.8;
          ctx.moveTo(px, py);
          ctx.lineTo(px + len, py + tilt);
        }
        ctx.stroke();

        const shockFlash = ctx.createLinearGradient(0, canvasEl.height * 0.22, 0, canvasEl.height * 0.78);
        shockFlash.addColorStop(0, "rgba(236, 248, 255, 0)");
        shockFlash.addColorStop(0.5, `rgba(228, 244, 255, ${0.03 + shock * 0.09})`);
        shockFlash.addColorStop(1, "rgba(228, 244, 255, 0)");
        ctx.fillStyle = shockFlash;
        ctx.fillRect(0, 0, canvasEl.width, canvasEl.height);

        ctx.strokeStyle = `rgba(224, 246, 255, ${0.05 + shock * 0.12})`;
        ctx.lineWidth = 1;
        const crestLines = 9;
        for (let i = 0; i < crestLines; i++) {
          const yy = canvasEl.height * (0.35 + (i / crestLines) * 0.5);
          const wob = Math.sin(now * 0.005 + i * 0.8) * (3 + shock * 6);
          ctx.beginPath();
          ctx.moveTo(wob, yy);
          ctx.bezierCurveTo(canvasEl.width * 0.3 + wob * 0.4, yy - 8, canvasEl.width * 0.7 + wob * 0.7, yy + 8, canvasEl.width + wob, yy);
          ctx.stroke();
        }
      }
